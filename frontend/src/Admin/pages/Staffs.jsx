...
export default function Staffs() {
    ...
    try {
      ...
      if (edit) {
        ...
        response = await axios.put(
          `http://localhost:5000/${process.env.REACT_APP_API_KEY}/api/staff/auth/updatestaff`,
          staffUpload,
          {
            headers: {
        ...
        response = await axios.post(
          `http://localhost:5000/${process.env.REACT_APP_API_KEY}/api/staff/auth/addstaff`,
          staffUpload,
          {
            headers: {
              "Content-Type": "multipart/form-data",
  ...
  const getStaff = async () => {
    try {
      const response = await axios.get(
        `http://localhost:5000/${process.env.REACT_APP_API_KEY}/api/staff/auth/getstaff`,
        {
          headers: {
            token: Cookies.get("auth"),
          },
  ...
  const handleDelete = async (delId) => {
    ...
    const response = await axios
      .delete(
        ` http://localhost:5000/${process.env.REACT_APP_API_KEY}/api/staff/auth/removestaff`,
        { data: id },
        {
          headers: {
            token: Cookies.get("auth"),
